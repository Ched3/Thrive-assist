...
def journal():
    ...
    if request.method == 'GET':
        ...
        package = db.execute('SELECT * FROM journals WHERE user_id = ? ORDER BY user_id DESC LIMIT 1', (user_id,)).fetchone()
        ...
        if not package:
            ...
            user_id, q1, a1, q2, a2, q3, a3, j = package
            print(q1, q2, q3)
            prompt = f"Journal: {j}\n{q1}\n{a1}\n{q2}\n{a2}\n{q3}\n{a3}"
...
def post_journal():
    ...
    if request.method == 'POST':
        ...
        db.execute('INSERT INTO journals (user_id, question1, question2, question3, answer1, answer2, answer3, journal) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', (session['user_id'], q1, q2, q3, a1, a2, a3, j))
        conn.commit()
        prompt = f"Journal: {j}\n{q1}\n{a1}\n{q2}\n{a2}\n{q3}\n{a3}"
        return jsonify({"response":chat(prompt)})
    else:
...
def history():
    if not session.get("loggedin", None) or not session.get("user_id", None):
        return redirect('/login')
    user_id = session['user_id']
    journals = db.execute('SELECT * FROM journals WHERE user_id = ?', (user_id,)).fetchall()
    print(journals)
